...
ggsave('Fig1_SI_plot.tiff', SI_plot_1, dpi = 500, width = 7, height = 4, units = 'in')
...
ggsave('Fig2_SEAb_plot.tiff', pred_SEA_B_ggplot, dpi = 500, width = 7, height = 4, units = 'in')
...
# turns out some of her output were for analysis with outliers while others were without

# make consistent with Fig. 5 format
(pred_SEA_B_ggplot2 <- ggplot(pred_SEA_B_df,aes(value, color = species, fill = species))
  + geom_vline(data = cred_intervals, aes(xintercept = mean, color = species), lwd = 1)
  + geom_vline(data = cred_intervals, aes(xintercept = ci_low, color = species), lty = 2, lwd = 0.9)
  + geom_vline(data = cred_intervals, aes(xintercept = ci_high, color = species), lty = 2, lwd = 0.9)
  ...
  + scale_pattern_density_discrete(range = c(0.5, 0.01), labels = c('Canada lynx', 'bobcat'))
  + scale_pattern_angle_discrete(range = c(-45, 45), labels = c('Canada lynx', 'bobcat'))
  + scale_x_continuous(name = expression("standard ellipse area " ('\u2030' ^2)))
...
ggsave('Fig2_SEAb_plot2.tiff', pred_SEA_B_ggplot2, dpi = 500, width = 7, height = 4, units = 'in')

# investigate correlation in niche breadth across regions
str(cred_intervals)
summary(cred_intervals)
cred_intervals_long <- cred_intervals %>% 
  pivot_wider(names_from = species, 
              values_from = c(mean, ci_low, ci_high))
summary(cred_intervals_long)

(SEA_B_correlation <- ggplot(cred_intervals_long, aes(x = mean_Lynx_canadensis, y = mean_Lynx_rufus))
  + geom_errorbar(aes(ymin = ci_low_Lynx_rufus, ymax = ci_high_Lynx_rufus), col = 'grey')
  + geom_errorbar(aes(xmin = ci_low_Lynx_canadensis, xmax = ci_high_Lynx_canadensis), col = 'grey')
  + geom_point(aes(fill = region, pch = region), cex = 3)
  ...
  + scale_x_continuous(name = expression("Posterior mean of standard ellipse area for Canada lynx " ('\u2030' ^2)), limits = c(0,2.5))
  + scale_y_continuous(name = expression("Posterior mean of standard ellipse area for bobcat " ('\u2030' ^2)), limits = c(0,5))
  + theme_classic2()
  + theme(legend.position = 'top', legend.text = element_text(size = 8),  
          legend.box.margin = margin(0, 0, 0, 0), legend.margin = margin(0, 0, 0, 0), legend.title = element_blank())
  + theme(axis.title = element_text(size = 12), axis.text = element_text(size = 10))
...
cor(cred_intervals_long$mean_Lynx_canadensis, cred_intervals_long$mean_Lynx_rufus, method ="pearson")

ggsave('Appendix_Fig5_SEAb_corr.tiff', SEA_B_correlation, dpi = 500, width = 5, height = 5, units = 'in')


# get relative differences for lynx and bobcat
str(pred_SEA_B_df)
head(pred_SEA_B_df)

SEA_B_foldchange <- pred_SEA_B_df %>% 
  group_by(species, region) %>% 
  summarize(mean = mean(value), sd = sd(value)) %>% 
  pivot_wider(names_from = species, values_from = c(mean, sd), names_sep = '.') %>% 
  mutate(mean_foldchange = median.Lynx_rufus/median.Lynx_canadensis)

summary(SEA_B_foldchange)
