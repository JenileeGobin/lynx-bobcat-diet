...
ggsave('Fig3_param_plot.tiff', param_dist_plot, dpi = 500, width = 7, height = 5, units = 'in')
...
param_diff <- all_param %>% 
  select(-parameter2) %>% 
  group_by(species, region, parameter) %>% 
  summarize(mean = mean(value)) %>% 
  pivot_wider(names_from = c(species, parameter), values_from = mean, names_sep = '.') %>% 
  mutate(d13C_diff = Lynx_rufus.d13c - Lynx_canadensis.d13c, 
         d15n_diff = Lynx_rufus.d15n - Lynx_canadensis.d15n) %>% 
  select(c(region, contains('d13C'), contains('d15n')))

summary(param_diff)
(lynx_d13C_diff <- max(param_diff$Lynx_canadensis.d13c) - min(param_diff$Lynx_canadensis.d13c))
(bobcat_d13C_diff <- max(param_diff$Lynx_rufus.d13c) - min(param_diff$Lynx_rufus.d13c))
(lynx_d15N_diff <- max(param_diff$Lynx_canadensis.d15n) - min(param_diff$Lynx_canadensis.d15n))
(bobcat_d15N_diff <- max(param_diff$Lynx_rufus.d15n) - min(param_diff$Lynx_rufus.d15n))

covariance_comparison <- all_param %>% 
  select(-parameter2) %>% 
  group_by(species, region, parameter) %>% 
  summarize(mean = mean(value)) %>% 
  pivot_wider(names_from = c(species, parameter), values_from = mean, names_sep = '.') %>% 
  mutate(d13C_diff = Lynx_rufus.d13c - Lynx_canadensis.d13c, 
         d15n_diff = Lynx_rufus.d15n - Lynx_canadensis.d15n) %>% 
  select(c(region, contains('covariance')))

summary(covariance_comparison)
...
ggsave('Fig4_overlap_plot_v1.tiff', overlap_plot1, dpi = 500, width = 6, height = 6, units = 'in')
...
ggsave('Fig4_overlap_plot_v2.tiff', overlap_plot2, dpi = 500, width = 7, height = 4, units = 'in')

# get summary of overlap

str(all_overlap)
overlap_summary <- all_overlap %>% 
  group_by(species_A, region) %>% 
  summarize(mean.mean = mean(mean), 
            mean.lowCI = mean(`2.5%`), 
            mean.highCI = mean(`97.5%`))
